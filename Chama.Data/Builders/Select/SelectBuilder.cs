...
namespace Chama.Data
{
	internal class SelectBuilder<TEntity> : ISelectBuilder<TEntity>
	{
        ...
        {
            return GetPreparedDbCommand().QuerySingle<dynamic>();
        }

        public string GetSql()
        {
            return Data.Command.Data.Context.Data.Provider.GetSqlForSelectBuilder(Data);
        }

        public TField QueryValue<TField>()
        {
            return GetPreparedDbCommand().QuerySingle<TField>();
        }

        public List<TField> QueryValues<TField>()
        {
